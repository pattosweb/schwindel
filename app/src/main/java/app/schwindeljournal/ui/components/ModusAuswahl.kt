    ...
    OutlinedCard(
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Text(text = modus.anzeigename(), style = MaterialTheme.typography.titleMedium)
            }
            Text(text = modus.buchtitel(), style = MaterialTheme.typography.labelMedium)
            Spacer(modifier = Modifier.padding(top = 4.dp))
            Text(text = modus.beschreibung(), style = MaterialTheme.typography.bodyMedium)
        }
    }
}
